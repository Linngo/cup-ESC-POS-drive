...
int 		SetlabelGap = 0;			/* Model number */
...
int p7[2] = { 0x01, 0 };
 ...
 const char	labelgap[] = { 0x1D, 0x6C, 0x04, 0x03};
/*
 * Prototypes...
 */
...
Setup(int argc, char *argv[])
{
  ...
  const char *label = "PaperType=";
  char cmd_cancellabel[] = { 0x1D, 0x6C, 0x05, 0x00};
  char cmd_setlabel[] = { 0x1D, 0x6C, 0x05, 0x01};
  if ( (optstr = strstr(argv[5], label)) != NULL)
  {
    switch(*(optstr + strlen(label)))
    {
      case 'L':
        SetlabelGap = 1;
        break;
    ...
    if (SetlabelGap) {
      pwrite(cmd_setlabel, 4);
      debug("is label\n");
    } else {
      pwrite(cmd_cancellabel, 4);
      debug("No label\n");
    }
  }
...
*/      
      if( (rasbufconst = rasbuf = malloc(header.cupsBytesPerLine * header.cupsHeight)) == NULL)
      {
        fprintf(stderr, "ERROR: Unable to allocate memory\n");
        break;
      }
      ...
      {
        if (cupsRasterReadPixels(ras, 
                                 rasbuf + y * header.cupsBytesPerLine, 
                                 header.cupsBytesPerLine) == 0)
          break;
      }
      if (bytesPerLine != header.cupsBytesPerLine) {
        for (y = 1; y < header.cupsHeight; y++) {
          for (i = 0; i < bytesPerLine; i++) {
            *(rasbuf + y * bytesPerLine + i) = *(rasbufconst + y * header.cupsBytesPerLine + i);
          }
        }
      }
      ...
      {
        i = 0;
        for(y = header.cupsHeight; y > 40; y -=40)
        {
          Height = y > 40? 40:y;
          rasbufhead[0] = 0x1D;
          rasbufhead[1] = 0x76;
          rasbufhead[2] = 0x30;
          rasbufhead[3] = 0x00;
          rasbufhead[4] = (unsigned char)(bytesPerLine & 0xff);
          rasbufhead[5] = (unsigned char)((bytesPerLine & 0xff00) >> 8);
          rasbufhead[6] = (unsigned char)( Height& 0xff);
          rasbufhead[7] = (unsigned char)((Height & 0xff00) >> 8);
          pwrite(rasbufhead, 8);
//          fflush(stdout);
          pwrite(rasbuf + i * bytesPerLine * Height, bytesPerLine * Height);
          i ++;
          fflush(stdout);
        }
      }
...
/*      if( ((rasbufconst = rasbuf = malloc(xsizealigned * header.cupsHeight)) == NULL) ||
      ...
      if( ((rasbufconst = rasbuf = malloc(xsizealigned * header.cupsHeight)) == NULL) ||
          ((sendbuf = malloc(bytesPerLine / 8 * header.cupsHeight )) == NULL) )
      {
        fprintf(stderr, "ERROR: Unable to allocate memory\n");
        break;
      }
      ...
      fprintf(stderr, "INFO raster: Printing page %d. Copies %d in %d\n", 
...
/*      for (coping = 1; (!Canceled) && (coping <= header.NumCopies); coping++)
      ...
      for (coping = 1; (!Canceled) && (coping <= header.NumCopies); coping++)
      {
        ...
        for(y = header.cupsHeight; y > 40; y -=40)
        {
          ...
          rasbufhead[2] = 0x30;
          rasbufhead[3] = 0x00;
          rasbufhead[4] = (unsigned char)(bytesPerLine / 8 & 0xff);
          rasbufhead[5] = (unsigned char)((bytesPerLine / 8 & 0xff00) >> 8);
          rasbufhead[6] = (unsigned char)( Height& 0xff);
          rasbufhead[7] = (unsigned char)((Height & 0xff00) >> 8);
          pwrite(rasbufhead, 8);
//          fflush(stdout);
          pwrite(sendbuf + i * bytesPerLine / 8 * Height, bytesPerLine / 8 * Height);
          i ++;
          fflush(stdout);
        }
      }
